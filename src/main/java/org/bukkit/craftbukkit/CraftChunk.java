...
public class CraftChunk implements Chunk {
	...
	public CraftChunk(final net.minecraft.world.chunk.Chunk chunk) {
		...
		this.worldServer = (WorldServer) this.getHandle().getWorld();
	...
	public World getWorld() {
		return ((IMixinWorld)this.worldServer).getWorld();
	}
	...
	public ChunkSnapshot getChunkSnapshot(final boolean includeMaxBlockY, final boolean includeBiome,
		...
		if (includeBiome || includeBiomeTempRain) {
			final BiomeProvider wcm = chunk.getWorld().getBiomeProvider();
			if (includeBiome) {
				biome = new Biome[256];
				for (int k = 0; k < 256; ++k) {
		...
		return new CraftChunkSnapshot(this.getX(), this.getZ(), world.getName(), world.getFullTime(), sectionBlockIDs,
				sectionBlockData, sectionSkyLights, sectionEmitLights, sectionEmpty, hmap, biome, biomeTemp, biomeRain);
	}

	public static ChunkSnapshot getEmptyChunkSnapshot(final int x, final int z, final CraftWorld world,
